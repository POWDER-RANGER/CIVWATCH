...
Scoring uses TF-IDF + Logistic Regression trained on civic-domain
keywords. CPU-only, no GPU required. Boots in < 2s.
"""
...
app = FastAPI(
  ...
  description="Civic document scoring, sentiment analysis, and anomaly detection",
  version="2.0.0",
)
...
_CONCERN_KEYWORDS = [
  ...
  "recusal", "exemption", "diversion", "ban", "sustained complaint",
]
...
class AnomalyRequest(BaseModel):
  scores: List[float] = Field(..., description="List of document scores")
  eps: float = Field(default=0.15, description="DBSCAN epsilon")
  min_samples: int = Field(default=3, description="DBSCAN min_samples")
...
def predict(item: SingleItem):
  ...
  result = _model.score(item.title, item.body)
  return ScoreResult(item_id=0, **result)
